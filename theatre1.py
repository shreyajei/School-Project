...
heading = tk.Label(master, text="THEATRE",font=('comicsans',25,'bold '),bg="light yellow")
heading.pack()
...
c.pack()
w=tk.Label(master, text="”ITS SHOWTIME”",font=("Times", "18", "bold italic"),bg="light yellow")  
w.pack()
...
if  date2<=today :
    ...
    def getdatacreate1():
        ...
        for (a,) in result:
            ...
            #mycursor.execute("CREATE TABLE theatre(name VARCHAR(30))")
            #Inserting Rows in to the table
            st="insert into theatre values('" + f + "')"
            print(st)
            mycursor.execute(st)
            button5 = tk.Button(master,text = "Join event ",font=("bold"),state=tk.NORMAL).place(x=900,y=400)
            popup()
...
def deletename():
    ...
    for (a,) in result:
        if (str(a)==str(f)):
            #mycursor.execute("CREATE TABLE theatre(name VARCHAR(30))")
            #Inserting Rows in to the table
            que="delete from theatre where name='"+ f +"'"
            print(que)
