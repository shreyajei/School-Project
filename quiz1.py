...
image = tk.PhotoImage(file="quiz.gif")
...
c.pack()
w=tk.Label(master, text="”Test Your Knowledge”",font=("Times", "18", "bold italic"),bg="light yellow")  
w.pack()
...
if  date2<=today :
    ...
    def getdatacreate1():
        ...
        for (a,) in result:
            ...
            #mycursor.execute("CREATE TABLE quiz(name VARCHAR(30))")
            #Inserting Rows in to the table
            st="insert into quiz values('" + f + "')"
            print(st)
            mycursor.execute(st)
            button5 = tk.Button(master,text = "Join event ",font=("bold"),state=tk.NORMAL).place(x=900,y=400)
            popup()
...
def deletename():
    ...
    for (a,) in result:
        if (str(a)==str(f)):
            #mycursor.execute("CREATE TABLE quiz(name VARCHAR(30))")
            #Inserting Rows in to the table
            que="delete from quiz where name='"+ f +"'"
            print(que)
...
def view():
    ...
    con = mysql.connector.connect(host="localhost", user="root", passwd="sql123",database="project")
    mycursor = con.cursor()
    sql_query = "select t1.* from studentinfo1 t1, quiz t2 where t1.username=t2.name;"
