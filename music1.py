...
heading = tk.Label(master, text="MUSIC",font=('comicsans',25,'bold '),bg="light yellow")
heading.pack()


image = tk.PhotoImage(file="music.gif")
...
c.pack()
w=tk.Label(master, text="”Where words fail, music speaks.”",font=("Times", "18", "bold italic"),bg="light yellow")  
w.pack()
...
if  date2<=today :
    ...
    def getdatacreate1():
        ...
        for (a,) in result:
            ...
            #mycursor.execute("CREATE TABLE music(name VARCHAR(30))")
            #Inserting Rows in to the table
            st="insert into music values('" + f + "')"
            print(st)
            mycursor.execute(st)
            button5 = tk.Button(master,text = "Join event ",font=("bold"),state=tk.NORMAL).place(x=900,y=400)
            popup()
...
def deletename():
    ...
    mycursor = con.cursor()
    sql_query = "SELECT * FROM muisc"
    mycursor.execute(sql_query)
    
    result = mycursor.fetchall()
    for (a,) in result:
        if (str(a)==str(f)):
            #mycursor.execute("CREATE TABLE muisc(name VARCHAR(30))")
            #Inserting Rows in to the table
            que="delete from music where name='"+ f +"'"
            print(que)
...
def view():
    ...
    con = mysql.connector.connect(host="localhost", user="root", passwd="sql123",database="project")
    mycursor = con.cursor()
    sql_query = "select t1.* from studentinfo1 t1, music t2 where t1.username=t2.name;"
